...
var GameMenuButtons = [
	...
	[ "save",		"Guardar estado.",						1,	1 ],
	[ "load",		"Cargar estado de guardado.",				1,	1 ],
	[ "autoread",	"Activar modo automático.",						3,	0 ],
	[ "gotonext",	"Ir a la siguiente opción o escena.",			3,	0 ],
	[ "returntop",	"Regresar a la pantalla de título.",							3,	0 ],
	[ "returnlist",	"Regresar al menú de interludios.",			2,	0 ],
	[ "status",		"Ver las habilidades de los sirvientes.",	1,	1 ],
	[ "arms",		"Ver el glosario de armas.",					1,	1 ],
	[ "history",	"Mostrar textos anteriores.",					3,	0 ],
	[ "graphic",	"Ver ilustración.",						3,	0 ],
	[ "config",		"Ajustar configuraciones.",					3,	1 ],
	[ "exitgame",	"Salir del juego.",							3,	0 ],
	[ "exitmenu",	"Salir del menú.",							3,	0 ]
];
...
class GameMenuLayer extends FadeLayer
{
	...
	function show(time=default_time, accel=default_accel)
	{
		...
		if(state=="hide")
		{
			if(sf.doubleCushionMenuOpen)	//	一度グラフィックのみ表示にする
			{
				...
				global.alternative_object.base.opacity = 0 if typeof global.alternative_object != "undefined" && global.alternative_object.base != void;	//	選択肢も
				visible	= true;
	...
	function showGameMenu(time, accel, changetextopacity)
	{
		dm("showGameMenu");
		setMode();
		kag.leftClickHook[0] = function { return true; };	//	左クリックを無効化
	...
	function hideGameMenu(time=default_time, accel=default_accel)
	{
		dm("hideGameMenu");
		removeMode();
		kag.leftClickHook.erase(0) if kag.leftClickHook.count > 0;	//	左クリックを元に戻す
...
//		dm("opacity = "+opacity+ " / current.opacity = "+window.current.opacity);
		...
		global.alternative_object.base.opacity = 255 - opacity if typeof global.alternative_object != "undefined" && global.alternative_object.base != void;	//	選択肢も
	}
